...
## simNN/README.md の章立て（★固定・2026-08-07 ユーザー決定）

**sim05 以降の `simNN/README.md` は次の6章構成に固定する**（雛形＝[TEMPLATE/README.md](TEMPLATE/README.md)）。
理由＝simNN README は「**今どこにいて次に何をするか**」を新セッションが1本で把握するための文書であり、
章立てが sim ごとに違うと毎回読み方を探すことになる（sim05 で実際に視認性の問題が出た＝ユーザー指摘）。

| 章 | 内容 | 外してはいけない点 |
|---|---|---|
| **① 主題と目的** | この sim が答える問いを1文＋編成＋閉じたい Cx | 主題が途中で変わったなら**いつ・何を根拠に**変わったかを1〜2行 |
| **② sim フローと現在位置** | 依存チェーン（G ゲート）＋**測定メニューの進行表**＋実施順の原則＋**役割分担表** | **★網羅的に**。ここだけ読めば現在位置が分かること |
| **③ 測定環境** | config（正の所在）・未確定 placeholder・**較正ボスとその選定理由**・実装済みの設計前提 | **幻獣枠は `_configSig` から復元できない**＝必ず明記（sim05 で欠落した） |
| **④ 実機走フローと測定メニュー** | 共通前提（記録様式）＋**Mx の目的・手順・走数・順序** | **手順の本体はここに置く**（`workspace/TODO.md` に置かない＝TODO は「次に何をするか」、README は「どうやるか」） |
| **⑤ クローズゲート** | 残ゲート・受入基準・golden 戦略 | **総ダメージ一致は受入基準にしない**（相殺するため何も保証しない） |
| **⑥ 更新履歴・md 相互参照** | 末尾ブロック（REPO_STANDARDS §4.1） | 「波及確認」列が本体 |
| （付録A） | **降格・retire した設計の隔離先**（任意） | 本文には**付録へのポインタだけ**を残す＝現在値と経緯を混ぜない |

- **不要な章は「該当なし」と1行書いて残す**（章番号をずらさない＝他文書からの参照が壊れるため）。
- **フォルダ構成・分析2層構造・ワークフローは本書が正**＝各 simNN README で複製しない。
- **sim01〜sim04 は凍結**＝遡及適用しない（履歴管理の原則1）。
...
| [sim05](sim05/README.md) | **ナポレオン**＋ヘカテー/テトラ/アリアンロッド/エレイン（configC） | **ナポ/アリアン編成のバースト成分別較正**（C40 追加ダメージの式 / C41 betaia cap / C44 バースト本体）。仕様確定=キャスパリーグ・最終検証=両面宿儺の2段構え | **進行中**: M1・M2 完了（C40 を編成横断＋敵横断で確定・C44 を分解・C42 確定・C47/C48 起票）／⏳ M3 実機測定中 → M5 → M4 → M6。**README は6章構成の先例** |
