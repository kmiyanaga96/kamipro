...
2. 実機試行ごとに **`data/record_skeleton.md` を複製して `data/trialNN.md`** を作成（原本・加工せず。冒頭にメタヘッダ: 日付/参照config/装備config名/敵HP開始状態）。**トライアルの複製・push はユーザーが行う**（テンプレは record_skeleton のみ）。
