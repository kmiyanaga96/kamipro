...
`config_sukuna_v2.json` の推奨順をリプレイし、各ターン開始時の buffCount を実測:
...
**この構造は buffCount が何を数えるかと独立**なので先行実装は可能だが、**今入れても挙動はほぼ変わらない**（全 tier 常時到達ゆえ、どのルールも「即撃ち」に縮退する）。∴ **試走 §B で tier 実発動データを取り buffCount を確定 → その後に実装**が正順（逆順だと直した効果が測れない）。
