...
>
> ### ⚠ 冒頭注記4（2026-08-07・**プレイヤーランクが M2 と M3 で違う**）
> **M2（trial01）= rank 173 ／ M3 以降 = rank 174**（ユーザー申告 2026-08-07・**装備はすべて不変**）。
> ∴ **§2.1 と冒頭注記の表示ATK のうち、ナポレオンの値（111143）は M2 時点のもの**。
> ⚠ **ランクボーナスは英霊にしか掛からない**（`src/app.js` `calcDisplayAtk` の `def.type==='hero'` 分岐）＝
> **神姫4人（86683 / 88993 / 90944 / 90045）は不変**。
> **影響は 10T 総ダメージで +15,943（×1.000006 ＝ 0.0006%）**＝実機ばらつき（holy ヒット間 CV 1.04%）の **1/1842**
> ＝**M2 と M3 の比較は成立する**。ハーネスは表示ATK を trial md ヘッダから読むので**コード修正は不要**。
> ⚠ マスターボーナスを割り振った場合は +20 では済まない（シムは非適用）＝**実機の表示値を記録すること**。
>
