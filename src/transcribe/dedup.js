// T1 録画転記 — 走査の重複除去と「持続性」の実測
//
// ★★本モジュールの現在の役割（2026-08-15 にユーザー承認で計画を組み替えた）:
//   ①**完全重複フレームの除去**（距離ちょうど 0＝同じフレームを二度デコードした分・情報損失ゼロ）
//   ②**持続性の実測**＝`LagProfile`（ラグ別距離 / セル |Δ| 分布 / `eventContrast` / **寿命 L**）
//
//   ⚠ **等間隔の間引き（stride）は使わない**。実測で **L ≦ 2 フレーム**＝`stride = floor(L/2) = 1`
//   ＝**間引きゼロ**にしかならないと確定したため。`EventDeduper` の stride 機構は
//   **将来の設計変更に備えて残置**するが、既定は `null`（＝間引かない）。
//
//   ★**削減の要求は P3-2（OCR 後のイベント畳み込み）へ移設された**。理由は計画側の定義そのもの＝
//   「**同一位置・同一値**の連続フレームを1イベントに畳む」の **「同一値」が値を読むこと＝OCR を要求している**。
//   ⚠ **要求を下げたのではない＝置き場所が間違っていた**（詳細は PHASE9_PLAN.md §4 P2 と P3-2）。
//
// ── ここに至るまでの経緯（同じ道を二度通らないための記録）────────────────
//
...
//
//   ∴ 問いを変えた: 「**取りこぼさないと保証できる最大の間引きはどれか**」。
//   ⭐ **等間隔の間引きには証明がある**: s フレームおきに採ると、任意の連続 s フレームの窓に
//      採用フレームがちょうど1枚入る。∴ **s フレーム以上表示され続けるものは必ず1回は捕まる**。
//      **検出が信用できない場面では、鈍いが証明のある方法を採る。**
//   ⚠⚠ ただし保証は **s ≦ 寿命 L** のときだけ成り立ち、**実測 L ≦ 2 だった**＝この路線は閉じた。
//
//   ★**寿命の測定器は3つ作って2つ捨てた**（いずれも出荷前に既知の正解で反証）:
//     ①ラグ曲線の膝                → 背景の動きと混ざる（撤去した `persistenceRatio` と同じ構造の誤り）
//     ②セル単位「次の jump まで」  → 真の L が 10/6/3 のすべてで同じ値を返した
//     ③✅ セル単位「jump 直後に |Δ|=0 で凍る長さ」（長さ1を除く）→ 真値を J によらず厳密に復元
//   ★**測定器は、既知の正解を動かして「値が追随するか」を見るまで信用しない。**
//
//   ⚠ **実測 L=2 は「数字が動いている」ことを意味しない**。ユーザー確定情報（2026-08-15）＝
//   ダメージ数値は「**わずかにフェードイン・消失があるが、位置は基本的に不動**」。
//   位置が不動なのに画素が凍らないのは、**数字の上に重なる演出**（P1 発見⑫のキラキラ）が
//   セルを動かし続けるため＝**読める長さは 2 より長いが、画素の凍結では測れない**。
//   ∴ 「同じポップアップか」の判定は**値を読んでから**（＝P3-2）が正しい。
//
...
export const DEDUP_DEFAULTS = {
  ...
  maxLag: 20,
  /**
   * 何フレームおきに採るか。**null＝間引かない（＝現在の確定した設計）**。
   * ⚠ **未了ではない**＝実測 L ≦ 2 で `floor(L/2) = 1` にしかならず、等間隔の間引きは無意味と確定した。
   * 機構は将来の設計変更に備えて残置する。
   */
  stride: null,
  /**
...
export function reportDedup(diag, sum, lag) {
  ...
  if (!sum.calibrated) {
    // ★2026-08-15 にユーザー承認で **WARN → INFO**。
    //   実測 L ≦ 2 ＝ `stride = floor(L/2) = 1`＝**等間隔の間引きは原理的に不可能**と確定し、
    //   削減は **P3-2（OCR 後のイベント畳み込み）** へ移設した。
    //   ∴ `stride` が未較正なのは**未了ではなく確定した設計**＝WARN で鳴らすのは誤り。
    diag.add('T1-DEDUP-001', 'INFO', {
      where: { stride: sum.stride, lifetimeFrames: lag?.lifetimeFrames ?? null },
      expected: '（確定）間引きはしない＝完全重複の除去のみ',
      got: `完全重複 ${sum.droppedDuplicates} 枚を除去／実測 L = ${lag?.lifetimeFrames ?? '決まらず'}`,
      hint: '★削減は P3-2（OCR 後・同一位置同一値の畳み込み）が担う。'
        + '⚠ `lags`（ROI 全体のラグ距離）から L を読んではいけない＝背景の動きと分離できていない。'
        + '⚠ ここで stride を推測で埋めない（P2-5 で同型の失敗を7回踏んだ）。',
    });
