...
| [`catastrophia_light`](catastrophia_light.md) | 光天獄カタストロフィア（カタス） | **`catas`**（キーは据置） | **メイン/サポート=加護 `weapon_amp:0.50` ＋ 攻撃枠 `box:{assault:1.0}`**／**サブ=属性枠 `box:{elem:0.30}`**（無条件）。★**configC（sim05 の較正編成）はメイン・サポートともにカタス**＝加護 1.0・assault +2.00（台帳 GEAR の逆算で裏取り済＝§2.3）。⚠ **加護は原文で `[レイ][シャイン][ルミナ][セイクリッド]` に限定**されているがシムは全スキルに一律適用＝**C50**。召喚攻撃は未モデル化 |
