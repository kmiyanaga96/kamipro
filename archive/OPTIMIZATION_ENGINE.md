...
本システムのエンジンは、**「極めて巨大な組み合わせ探索空間」**から、数十秒の探索時間で実機とほぼ同等の最適アビリティ順序を導き出すため、以下のハイブリッド設計を採用しています。
...
- 例えば、エジソンの攻撃ロボ（`droid`）、補助ロボ（`banoshik`）、武器バフ（`amplifa`）などの起点となるアビリティの組み合わせ（prefix）を列挙します。
- この時点で数十本の「開幕初期アビリティ列」のルート候補が作られます。

### 2.2 静的proxyスコアリング (`_staticPrefixDmg`)
列挙された全prefixに対して、いきなり本格的な「ビーム探索（全ターンシミュレーション）」を行うと非常に重くなります。そこで、**「静的greedy（貪欲法）」**（proxy）を用いて各ルートを高速に評価します。
- `_staticPrefixDmg` は、T1で開幕prefixを強制適用した後、残りのターンを **ビーム探索なしの静的greedy**（各ステップで最もスコアが高い1手だけを即座に選ぶ `_stepStatic`）で最後まで完遂し、その総ダメージをルートの仮スコアとします。ビームを使わないため1ルートあたり数ミリ秒で完了します。
...
- **定石度（orthodoxy）の動的評価**:
  `T.orthodoxy`（定石スコア）は、特定のバフ・デバフが有効な状態でダメージ行動を重ねた場合に加点される形で動的に計算されます。具体的には、以下のバフの有無をチェックし、各バフが有効なら +1 を加算します：
  - **バースト実行時**: `divinus_def`（テトラの防御デバフ）、`effond_def`（エレインの防御デバフ）、`absolute`（テトラのバフ）、`nights`（ヘカテのバフ）の各有効状態
  - **テトラのジャッジメント（`judg` ph0）実行時**: `divinus_def`、`effond_def` の各有効状態
  - **エレインのエフォンド（`effond`）実行時**: `divinus_def` の有効状態
  このスコアは、探索の枝刈りにおける多様性判定の優先シグナルとしてのみ使われ、最終的な火力評価（目的関数）には一切干渉しません。
- **ダメージの単調増加（品質保証）**:
...
- **状態のフラット数値化**: 各キャラクターの累積バフや契晶などの状態は、参照共有を防ぎつつクローンコストを最小にするため、**すべて平坦な数値変数**（オブジェクトではない）として `Sim` クラスのインスタンス直下に宣言されています。クローン時はスプレッド構文（`{...this.g}`）で浅いコピーを行い、バフ配列だけは `.slice()` で独立複製することで、高速かつ安全な状態の分岐を実現しています。
...
### 5.1 状態のクローン (`clone`)
探索が分岐する際、現在の `Sim` インスタンスの `clone()` メソッドが呼ばれ、独立した複製が生成されます。
- ギア倍率（`g`）やCD辞書（`cd`）はスプレッド構文（`{...this.g}`）で浅コピー。
- バフ辞書（`buf`）は各バフキーの配列を `.slice()` で個別に複製（配列の中身は数値なので浅コピーで十分）。
- キャラ固有の累積状態（契晶・連理魔力・ロボ残ターンなど）はフラットな数値変数なので、単純な代入（`s[k] = this[k]`）で瞬時にコピー。

### 5.2 アビリティの発動 (`use` と候補の `exec`)
探索エンジンがアビリティを実行する際、2つの処理が連携して動作します。
- **`use(name, T, ord)`**（共通の帳簿処理）: アビリティの CD（クールタイム）をセットし、契晶コストを消費し、ターン内のアビリティ使用回数 `T.ability` をインクリメントします。さらに、パーティ全員のキャラクター固有フック `onAbility` を呼び出し（エジソンのロボ反応やテトラの連理魔力蓄積などがここで発火）、行動ログ（`ord`）に記録します。
- **候補の `exec` クロージャ**（個別の効果処理）: キャラクター定義（`CHAR_REGISTRY`）の `cands` に定義された `exec` 関数が、バフの付与や特殊効果の適用を行います。`exec` が未定義の場合は、上記の `use` のみが実行されます。
...
ダメージ計算は、神姫PROJECTの実機仕様に合わせた「区分線形減衰モデル」を採用しています。ダメージが特定の上限（Cap）に達すると、超過分の寄与率（Slope）が段階的に減少します。フレーム（ダメージ種別）ごとに減衰の仕組みが異なります。
- **通常攻撃（`na`）**: 3段階の上限を持ちます。第1上限（`cap1`、ギアの `na_cap` で強化可能）を超えた分は ×0.5、さらに第2上限（cap1 + 10万）を超えた分も ×0.5、第3上限（cap1 + 20万）を超えた分も ×0.5 と、「半減を3回反復適用」する多段構造です。
- **バースト（`burst`）**: 単一の上限（100万、ギアの `burst_cap` で強化可能）を持ち、超過分の寄与率は **0.10（1/10）** です。
- **バーストストリーク（`streak`）**: バーストチェインの参加人数に応じた2段階の閾値を持ち、第1閾値超過分は slope1（0.25）、第2閾値超過分は slope2（0.40）で減衰します。
- **アビリティダメージ（`abi`）**: 各アビリティ固有の上限を持ち、超過分の寄与率は **0.04（1/25）** です。
...
*   **アロケフリー (Allocation-free) 設計**
    *   プログラムが動いている最中に、新しくメモリ領域を確保（アロケーション）する処理を行わない設計。JavaScriptでは、メモリ確保を頻繁に行うと後述の「GC」が誘発されて画面がフリーズするため、配列やオブジェクトを事前に1つ作っておき、それを中身だけ書き換えて再利用します。
