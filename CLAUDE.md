...
| 5 | `judg_calib=0.62`（C30・エジソン×configB fit） | judg ph0 の過大を是正 | ナポ/アリアン×宿儺では judg ph0 が **×1.36 不足**＝過剰に効いている（C44） |

**⚠ ただし「必ず外れる」わけでもない（2026-08-03・肯定側の実例）**: sim04 の `calib_burst=2.07` は、ナポ/アリアン×両面宿儺の強制リプレイで**バースト本体 ×1.04（1バーストあたり ×0.98）＝転移していた**。
∴ 原則は「**転移するかどうかを測る**」であって「転移しないと決めつける」ではない。**測れば安いものから測る**（成分別の強制リプレイは約3秒）。
...
- **バックログ状態（open のみ・fixed/wontfix/closed の詳細と根拠は CALIBRATION_ANALYSIS.md が正）**:
  - **C25**（★絶対値較正の本丸・open/再定式化・**2026-08-03 成分別に分解**）: 乖離は pre-cap raw に局在（cap/slope は飽和一致で妥当）。sim04 で frame calib（calib_na/burst/judg）確定。**sim05 pre-trial の強制リプレイで バースト本体は ×1.04＝一致**（`calib_burst=2.07` は本編成へ転移）＝**一律スカラの追加は不要**。不足は **betaia（C41）/ バースト追加ダメージ（C40）/ holy** に局在。
  ...
  - **C40 / C41**（**★sim05 pre-trial で起票 2026-08-03・T1 乖離の 77.5%**）: **C40=バースト「追加ダメージ」の定式化が違う**（シムは cap 非拘束なのに実機の 1/4〜1/7＝**cap 引上げでは解けない**。実機は**本体の 0.51〜0.77 倍**・4キャラ21バーストが同帯） ／ **C41=`DMG.betaia_cap`=800,000 が過小**（シム raw 281万を硬capが34%切り捨て・実機1ヒットは 206〜247万で**ターン毎に変動＝cap 非拘束**）。**いずれも実装は本trial待ち**（定式化の切り分け／cap を上げるか外すか）。根拠 `simulation/sim05/analysis/`。
  ...
  - **C42 / C43 / C44**（同上・open）: **C42=同ターン発動回数の上限が実機より厳しい**（alone 2 vs 実機3・holy 6 vs 実機7・legend 2 vs 実機3。⏳契晶記録待ち） ／ **C43=アビ上限超過ペナルティが「硬い剪定」**（実機は**超えられる**＝ペナルティはバフ消去のトレードオフ。実機は T1 に 42手を通した。⏳消去範囲の実機観測待ち・既定敵は golden 影響なし） ／ **C44=`judg_calib=0.62` の編成非転移**（judg ph0 が ×1.36 不足・CLAUDE.md §5 の5例目）。
