...
- **バックログ状態（open のみ・fixed/wontfix/closed の詳細と根拠は CALIBRATION_ANALYSIS.md が正）**:
  ...
  - **C37**（open・**起票のみ／実装しない**）: **探索パラメータ `BEAM_W=64` は編成依存**＝ナポ/アリアン×宿儺では最悪点で、幅を変えると総ダメが **+3.0〜+5.6% 変動しかつ非単調**（BW384=+5.64%）。根拠の C16 はエジソン編成の測定値で本編成へ転移せず。本質は幅不足でなく**枝刈りの代理採点（ロールアウト静的greedy）**＝近似最適順どうしの優劣が識別できていない。**実験1（全17prefix実測）: `PREFIX_TOPK=8` は転移確認＝損失0.017%・実害なし**（prefix分布は3層・上位6本の幅わずか0.13%＝入口の分岐でなく道中の枝刈りが効いている裏付け）。残 `BEAM_DIVERSITY_K` 未測定・**`CALIB_GRID`={judg,pactcore,effond} はナポ/アリアンのアビを含まず＝新編成の主役2人の静的スコアを較正できない構造的欠落**。⚠**Phase 7 のクローズ根拠（§7.2「BW64 が大域最良＝実用上限」）もエジソン依存と判明＝再評価の対象に戻った**。一般則: **編成を移したら探索パラメータは再測する**。
