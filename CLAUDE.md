...
- **次セッションの申し送り（優先順）**:
  ...
  3. ✅ **sim05（ナポ/アリアン移行＋追撃較正）計画策定済み（2026-07-22・`simulation/sim05/README.md`・実行未着手）**: 読み順は sim05/README §0。依存チェーン G0〜G5＝コード整備（ナポ押し順cands.s・アリアン条件付き+1）→configC→追撃C3/C5 fit→override/golden戦略。**Q1/Q4/Q5 回答反映済み**（Q1=一次情報が正＝1roy/2pike≥15/3consort≥20/4factor≥10・Q4=案B希望だがedison再帰余地→**二重fixture golden 推奨**・Q5=上限超過は複数回＝holy実機〜5回→累積カウンタ）。**残ゲート=Q2（ナポ+アリアンはcath_palugをT1撃破の公算→高HP反復ボスを新規選定）・Q3（configC後日・ウェポン更に強化）**。G1コード整備はQ1/Q5確定で着手可。intake（編成/実機ATK/HP/所感）は `simulation/napoleon_arian_migration/README.md`。
  4. **並行/後続**: アリアン残作業（A8急所倍率・**絶対値fit＝sim05でアリアン入り環境に相乗り**・§1.2とA0の係数矛盾5.5/3000 vs 5.0/2500 再確認）。C24（ゲージ）は較正走相乗りで診断更新。CHARACTER_ANALYSIS §2（ヤマトvsアリアン）再測。
