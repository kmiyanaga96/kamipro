...
- **DOC_RELATION_PLAN.md**: **md 相互参照の整備計画（2026-08-05 起草・★計画のみ／ユーザー承認まで着手禁止）**。md 間の更新漏れインシデントを根絶するため、①参照の機械検証可能化（`tools/doc_refs.mjs`）②単一の正の徹底（統一文言）③大テンプレ末尾の更新履歴＋被参照ブロック（自動生成）④用途別の小テンプレ、を段階計画 S1〜S5 で進める。**実測＝md 163本・§4ヘッダ充足 9.8%・リンク切れ約50件・曖昧参照53件**。⚠**§2 が要点＝末尾テンプレ単体で消えるインシデント型はゼロ**（効くのは機械検査と単一の正）。§8 に未決6件（要ユーザー判断）。**§7＝常駐サブタスク**（イベント駆動＋Nセッション定期・機械が経過を数える）。
