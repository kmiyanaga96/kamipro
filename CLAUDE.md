...
- **.agents/AGENTS.md**: Antigravity（Gemini）エージェント用のルール／ガイドライン定義。開発不変条件（循環インポートにおける遅延評価規律・Worker用export）・Gitワークフロー・検証ゲート（raw 174,697,325 / calibrated 206,846,142）を規定。
...
- **Phase3-1 事前計算マップ（ホットパス高速化・実装済）**: `buildFormation` で `ABIL_KEYS`/`ABIL_KC`/`ABIL_CANDS`/`ABIL_BASE_S` を一度だけ構築し、`_stepStatic`/`_candidates` が `Object.entries(ABIL)`・ネスト参照・`computeBaseScore` 再計算をせず `ABIL_KEYS` を1パス走査する。**⚠不変条件**: 走査順は `ABIL` 挿入順（=`Object.keys`順）でタイブレークは厳密 `>`（先頭最大）。キャラ追加・`abilities`/`cands` 変更時はこのマップ構築を経由するため自動追従するが、**走査順や `>` 比較を崩すと最適押し順の選択がズレる**（ゴールデン値 raw 174,697,325 / calibrated 206,846,142 で検証すること）。
...
- **作業開始時**
  ...
  2. 作業用ブランチを切って開発を開始。
- **検証 (作業完了時)**
  - 必ず `npm run test:golden`（期待値: raw 174,697,325 / calibrated 206,846,142）を実行しパスを確認。
- **反映・プッシュ**
...
4. **自律修正とテスト**: 実装担当（Claude Code）が `design_report.md` を検証して `simNN/integrated_analysis.md` にまとめ、コード修正後テスト実行。期待値 `raw 174,697,325 / calibrated 206,846,142` と追加検証ケースのパスを確認。
...
- FullBurst: `10/10`
- TotalDmg（raw・較正なし＝①funki修正モデル）: `174,697,325`
- TotalDmg（**calibrated**・自動較正 `{judg:122,pactcore:1,effond:93}` 適用＝production 出荷値・BW64で再fit）: `206,846,142`
...
- **バックログ状態**: C1=open / C2=open / C3=investigating / C4=fixed / C5=fixed / C6=wontfix / C7=deferred / C8=fixed / C9=fixed / C10=Phase5昇格 / C11=fixed / C12=fixed (案C＝ビーム多様性枠で定石枝を保持、④b＝tenyaをatomicとreに分割しinterleave化、②定石タイブレーク) / **C13=fixed（リプレイ往復スキップ=tenya_re・commit済）/ C14=fixed（①funki解禁バグ＝毎ターン化を恒久実装・C15較正とセット確定）/ C15=closed（探索rolloutポリシー脆弱性＝案(c)自動較正を production 実装＝§6。judg×pactcore×effond 3変数較正＋粗→細2段＋step調整/gear汎化まで完了・joint最適で+18.3%。**次セッション=演算高速化**）/ **C16=進行中（演算高速化）。第1手＝`BEAM_W` 128→64（`constants.js`）：較正rollout改善後は幅が品質を買わず逆効果（非単調の崖）。他ギア退行なし(burst −0.046%/abi −0.014%)・FB10/10・本探索/較正full-verifyの両phaseに効き約1.4倍高速。**⚠BW64では較正が新winnerを再fit**＝generic は {judg:145,effond:100}(206,622,997)より {judg:122,effond:93} が優位(+0.108%)＝production出荷値 206,846,142。縮小ホライズン較正は棄却(generic n=6でoverride化け−0.112%)。第2手＝`PREFIX_TOPK` 10→8(PoC安全床・本探索タスク約18%減・BW64で真勝者=baseline/loss0%実測・golden不変)。第3手＝持続化Increment1(A+B土台)＝探索結果キャッシュ(ヒント検証)：行に採用キー列を保存し config署名でキャッシュ、命中時は現行エンジンで決定的リプレイ→総ダメージ照合OKなら探索/較正を丸ごとskip(不一致=エンジン差/改ざんで破棄)。ダメージは押し順で決まりoverride非依存。`greedyTakeTurn(t,forcedKeys)`/`_replayResult`(sim.js)＋`tryResultCache`/`storeResult`/`ENGINE_VERSION`(app.js)・runSim/fallback両経路に配線。ヘッドレス検証済(golden不変・override非依存・改ざん検知)・**worker実経路はブラウザ最終確認要**。第4手＝持続化Increment3(越境B=JSON入出力)＝`exportResultCache`/`importResultCache`(app.js)＋UIボタン(index.html「キャッシュ書出/取込」)。実UI実行の本物configSigをそのまま持ち運ぶ(correct by construction)ためInc2のconfigSig再現問題を回避。キー=`ENGINE_VERSION|configSig`込み保存で別版は自然無視・命中はリプレイ検証で担保。シリアライズcoreヘッドレス検証済(round-trip/不正JSON拒否/golden不変)・**UIボタンはブラウザ確認済**。第5手＝火力指数の分母を素直押し(自然s)へ戻す＝C15の「baselineにも較正override」を逆転(worker.js/_fallbackRunSim)。分子(opt)のみ較正・分母は素直押しにし、指数=最適÷素直押しが較正の火力貢献も含む「最適化の全価値」を表す(generic 146.1・S級/従来112は較正の恩恵を分母に混ぜ過小化)。ダメージ・golden不変・ENGINE_VERSION=r2で旧キャッシュのbaseDmg無効化。**Increment2(プリコンピュート同梱)は保留**＝configSigがGEAR/subs/enemyを含みUI状態依存(headless既定はCURRENT_SUBS=[]だがUI applyGearは['','']等)のため実行時UI値の忠実再現とユーザーの頻用config manifest指定が必要）**。現ゴールデン値=**raw 174,697,325 / calibrated（自動較正 {judg:122,pactcore:1,effond:93}・BW64）206,846,142**（SEARCH_ROLLOUT_DESIGN.md §6）。
