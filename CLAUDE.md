...
- **ジャッジ/奮起/ヘカテー3アビの「即使用可」フラグは非蓄積**。CDを0にリセットする二値制御で表現し、
  トークン蓄積による連続使用は不可。
- **ジャッジは温存せず使用可になり次第即発動**（`while(cd.judg===0)`で発動）。連理procによる
  再発動は`JUDG_REACT=4`回まで（procRで`cd.judg>0`時のみリセットしカウント）。
...
T2 FB:5 J:4 renri:10   T7  FB:5 J:4 renri:35
T3 FB:5 J:4 renri:15   T8  FB:5 J:4 renri:40
T4 FB:5 J:4 renri:20   T9  FB:5 J:4 renri:44
T5 FB:5 J:4 renri:25   T10 FB:5 J:4 renri:49
```
（J:5はT1のみ＝戦闘開始時judg使用可。以降は再発動4回でJ:4）
