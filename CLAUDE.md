...
- **sim03 準備完了（2026-07-13 本セッション）**:
  1. **敵DB登録済み**: sim03較正ボス＝**フィンブルヴェトル**（`fimbulvetr`・幻・affinity=1.5確定・def/max_hp placeholder→sim03で推定）。一次情報 `enemies/fimbulvetr.md`（実機所感・敵行動全文を保全）。
  2. **プロトコルv2確定**（`simulation/sim03/README.md`・ユーザー承認2点反映）: **P1〜P3採用・P4（crit-off）は第2走の弱ボス（中立属性推奨）へ延期**＝フィンブルは強敵で装備DG非現実的→full-crit config A専任。**段階式配分＝第1バッチ K3走（撃破反復・粗ログ）＋P3走（部分深測定T1〜T3・hit毎）→中間分析ゲート→第2バッチ配分**。固定押し順は**ユーザー実績ライン**（シム推奨順はゲージDOWN未モデル化で実行不能リスク→replay机上照合後に採用判断）。
  3. ボス固有の設計制約を織込済み（sim03/README §3.0）: ディスペル3種＝stack=0自然実験（C25 baseline分解の贈り物）／系統交絡3種（敵防御UPドリフト→毎ターンFB維持が較正衛生・味方攻撃DOWN・旺盛×HP結合仮説H2）は記録で中和／ゲージDOWN≈−100は実測対象（H1・新Cx候補）／full-crit決定的＝分散分解が主レバー（少数試行で足りる）。
- **次セッションの申し送り（優先順）**:
  1. **ゲート2**: C23済みエンジンでフィンブルヴェトル探索→キャッシュexportを `sim03/data/configA.json` へ（storeResultはdispAtk同梱済み）。第1バッチのブロッカーではない（固定順=実績ライン）が、replay照合に必要＝早期取得。
  2. **実機第1バッチ受領**（K3+P3・記録項目= sim03/README §3.2・trialXXメタヘッダ必須）→ 定量/定性/統合のフロー厳守で分析（**情報の多さと正確さ優先**・quantitativeは手法/スクリプト/入力の明記必須=P3）。
  3. 中間分析ゲート（P間決定性・ボス挙動安定性・max_hp収束・シム順ゲージ実行可能性）→第2バッチ配分（B=境界深測定の投入など）。
  4. 較正順: C25 raw枠帰属→C5追撃cap再較正（C3統合）→絶対レベル（affinity=1.5既知・隠れ耐性なし仮定は §3.0-7 に明記）→def/耐性の定量化→golden再fit。
  5. C24 の実機確認2点（補助ロボ黄反応のゲージ増分・エレイン1のゲージ仕様）は §3.2 深測定項目に、C23追検証（各ターン初judgフェーズ）は共通記録に組込済み。
