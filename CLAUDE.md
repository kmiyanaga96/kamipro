...
- **次セッションの申し送り（優先順）**:
  1. **ゲート2 再export**: 現 `sim03/data/configA.json` は**フィンブル版＝無効**。C27エンジンで**キャスパリーグ（闇/affinity1.5・def10/HP1億placeholder）を敵に選び直して再探索→re-export**（装備不変＝`configA_gear_panel.md`継続・gear/dispAtk再利用可）。
  2. **固定押し順の確定**（ユーザー判断・sim03 §3.1）: シム推奨順（推奨）or 実績ライン。**第1バッチ配分の確定**（推奨 D×5→中間分析）。※前ターンのAskUserQuestionはツール障害で未回答＝要再確認。
  3. **実機第1バッチ受領**（D×5・記録項目= sim03/README §3.2・全hit毎・T1終了HP%必記・trialメタヘッダ必須）→ 定量/定性/統合のフロー厳守で分析（**情報の多さと正確さ優先**・P3）。
  4. 中間分析ゲート（決定性・ボス挙動安定性・max_hp収束・ディスペル一貫性）→第2バッチ配分（序数A/B・追加走など）。
  5. 較正順: **絶対レベル（キャスパリーグ=ライトレジスト光非適用でボス耐性=1保証＝本命アンカー）**→C25 raw枠帰属→C5追撃cap再較正（C3統合）→def/耐性→golden再fit。C24の2点・C23追検証は §3.2 に組込済み。
