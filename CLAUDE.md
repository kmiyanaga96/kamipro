...
- **AGENTS.md**: 全エージェント共通エントリポイント。本書を一次情報として参照しつつ、「破るとUI全消失/フリーズ」のハード不変条件と検証ゲートのみ抜粋（クロスツール規約の入口）。
...
- **ROADMAP.md**: 長期ビジョン（敵行動・味方生存・Phase 4 実機較正と最適押し順改善の泥沼）＋新キャラ導入ワークフロー構想。
