...
- **バックログ状態**:
  ...
  - C25: open (**★絶対値較正の本丸: 減衰capが会心・急所の「内側」なのが構造誤りの疑い**。証拠=streak(会心不可)だけ100%完全一致・会心付き成分は×1.5〜2級超過。B1のnaB 2×過小もこれで説明。フラットスカラ/def単独fitはB4で棄却=def=10維持)
  - C26: open (**試行2探索がエジソン英霊武器追撃OFF(mult=0)で走っていた確定バグ**=fidelity完全一致が証明。実機は同成分445万〜1001万/T。キャッシュキーがmult非含の設計欠陥併発。ユーザー確認1点=UI武器スロット0設定有無)
  - C3: investigating
...
- **次アクション優先順**: 1) **C26ユーザー確認**（UI武器スロット0=ランチャータンク設定有無）→追撃ONで再探索＋`_configSig`拡張 → 2) **C25検証**（非会心非急所サンプル照合→cap×会心順序の試作モデル→golden再fit） → 3) C23/C24特定 → 4) def再fit→kill-turnゲート再判定（現状は成分×1.5〜3.7級乖離で**S1+S2保留継続**、ただし構造特定済みで±10%到達は射程内） → 5) sim03（T3拡張）は**C25決着後**。
