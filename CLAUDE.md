...
| [tools/skills/README.md](./tools/skills/README.md) | **カスタムスキルの実体**（`check-engine-invariants` / `run-sim-experiment` / `sync-workspace-handoff` / `verify-transcribe-pipeline`）。定義は `.claude/skills/<name>/`・実体は `tools/skills/*.mjs` の2層。**判断はスキル側・転記と検算はツール側** |
