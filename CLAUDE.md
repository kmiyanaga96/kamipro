...
- **次セッションの申し送り（優先順）**:
  1. **sim04 開始ゲート（ユーザー作業）**: 強化後装備で configB export＋`configB_gear_panel.md` パネル転記＋表示ATK/HPスナップ報告（`DISPLAY_ATK_OVERRIDE`/`DISPLAY_HP_OVERRIDE` 更新）。詳細 `simulation/sim04/README.md` §2。
  2. **sim04 実施（Opusセッション）**: 読み順は sim04/README §0。構造修正（C31〜C35）→M走データ取得→fit（C25→C5/C3→C30）→golden/override再fit→新推奨順export＋序数diff記録。**構造修正とスカラfitの間でセッションを切らない**。
