...
| **C50** | 幻獣の加護（`weapon_amp`）を**全ウェポンスキルに一律適用**（一次情報では対象スキルが限定されうる）。configC は `weaponAmp=1.0`＝**全 GEAR 枠が ×2.0** | カタスの加護テキスト原文（`gamedata/md/幻獣/catastrophia_light.md` §1.2）。⚠golden 非影響 |
