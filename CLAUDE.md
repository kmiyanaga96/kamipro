...
期待値（基準・DMG定数が現行値の場合）:
```
T1  FB:5 J:3 renri:5  dmg:5,217,896    T6  FB:5 J:4 renri:30 dmg:47,016,826
T2  FB:5 J:4 renri:10 dmg:14,835,564   T7  FB:5 J:6 renri:30 dmg:59,500,651
T3  FB:5 J:3 renri:15 dmg:24,861,482   T8  FB:5 J:4 renri:30 dmg:70,473,284
T4  FB:5 J:2 renri:20 dmg:31,488,482   T9  FB:5 J:4 renri:30 dmg:81,398,743
T5  FB:5 J:3 renri:25 dmg:39,345,483   T10 FB:5 J:4 renri:30 dmg:100,732,675
```
（FullBurst:10/10、TotalDmg:100,732,675。`DMG` 定数を変えると数値も変わる＝この基準も更新する。
...
参加人数非依存でストリーク×1.1・減衰は1.1倍後に計算＝`_decay('streak')`前に乗算で構造一致。サブ編成変更時は要更新)。
減衰(`decay_streak.caps`)は参加人数別の第一/第二減衰(raw実ダメ閾値)で、
...
`burst_followup_mult`を3→11・`bplus_arrive`を50万→62.4万に実機較正(現神の祈りのみ・単体敵・中立属性・エジソン/エレイン/テトラ3点フィット・2026-06):
Number1=core+ARRIVE flat(全キャラ)、Number2=追撃倍率×naB(全キャラ)の2ヒット構造を確認。
較正式: naB_el=107,886/naB_t=149,029よりm=(B_el×8.75-B_t×8.75)/(B_el-B_t)×(1/8.75)→m≈10.94、
...
**追撃の構造修正(2026-06)**: 「追加ダメージは各キャラ固有のバースト効果(`onBurst`)として内包される」が正しい仕様。
`burst()`に置いた全キャラ共通のinoriゲート追撃は誤り→廃止。ヤマトの追加ダメージはヤマト`onBurst`内で
`buf.inori_burst`ゲートして実行するよう移動。他キャラ固有の追加ダメージ(ヘカテー/テトラ)は各自の`onBurst`に元々存在し影響なし。
基準値106,390,396→100,732,675へ更新(誤加算の除去が支配的)。
**残課題**: スクショ3倍 vs 実測11倍の桁差(多段ヒット/表示仕様の可能性)・追撃の味方全体スコープの最終確認(no-inoriフルバーストで全員の第2数値消失を確認すれば確定)。
