...
| 日付 | 変更点 | 波及確認 |
|---|---|---|
| 2026-08-23 | **★★P3-1b を実走フレーム5枚で実測＝0/5（1つも読めない）**＋**`TOTAL` の訂正を受領**（[gamedata/md/その他/burst_display.md](./gamedata/md/その他/burst_display.md) §1.2） | ★**マスクの効果は測れていない**＝ブロッカーは上流の「**文字の位置が見つけられない**」。行の射影が返す帯は **200〜705px**（実際の数字行は約90px）／ラベルの全面探索も**人物画に score 0.729**／根っこは**`glyphMask` がグリフに選択的でない**（インク率 **19〜53%**）。⚠ 行の問題は **P3-1 発見②で既知**だったが教示経路は**ユーザーが囲む**ことで迂回しており、**自動化の段で初めて binding した**。★★**訂正の波及＝`TOTAL` に追加ダメージは含まれない**＝`追加 = TOTAL − 本体` は成立せず、**C40 / C47 の 2026-08-14 追記を無効化**（測定経路が1つ失われた・両台帳に反映）。実装変更なし＝**golden 3/3 に影響なし** |
| 2026-08-23 | **一次情報を受領＝バースト本体の演出は3型**（[gamedata/md/その他/burst_display.md](./gamedata/md/その他/burst_display.md) 新設）。①単発＝ナポレオン/テトラ（**`TOTAL` は出る**）／②2発＝アリアンロッド／③多段＝ヘカテー/エレイン（**重なりあり**）。PHASE9_PLAN §4.3.1 の依頼 1c を**型ごとの取得計画**へ更新 | ★**取得計画が変わった**＝**重なるのは③だけ**なので**型を混ぜずに集める**。★1 多段＝マスクが**効くか** ／ ★2 単発＝マスクが**壊さないか**（対照＝誤検出で読める数字を潰す害は「効くか」より先に確かめる）。⚠ **1枚に全ヒットは写らない**＝**検算⑦は1フレームでは閉じない**（P3-2 の仕事）。⏳ 単発型で **`TOTAL` が本体と一致するか大きいか**は **C40 の直撃**（sim05 凍結中につき記録のみ）。実装変更なし＝**golden 3/3 に影響なし** |
