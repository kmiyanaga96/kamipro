...
- **次セッションの申し送り（優先順）**:
  ...
  3. ✅ **sim05（ナポ/アリアン移行＋追撃較正）＝策定完了・G1コード整備＋構造修正済み・実行はconfigC/ボス生存確認待ち（2026-07-22 大幅進展）**: 読み順 `simulation/sim05/README.md` §0。
     ...
     - **残ゲート**: Q3＝configC（月末・ウェポン更に強化）／**生存・討伐再現性**（両面宿儺は全滅時計 鬼の魔力≥10・基礎火力/回復/状態異常回復不足でT2/3全滅濃厚＝ナポ/アリアンが安定5走できるか実機確認。全滅頻発なら強機獣へ）。
  4. **★次セッションの実装2本（ユーザー指示2026-07-22・コンテキスト増大につき申し送り／どちらもgolden不変見込み）**:
     ...
     - **(B) 両面宿儺由来の修正（sim05前タスク）**: ①**鬼神障壁**＝敵側final-dmg枠別cap（enemy特性 `barrier:{na:6e6,burst:3e7,streak:3e6,abi:?,rate:0.70}`＋ダメージ確定後に枠別クランプ・C36味方側capと対）。②**アビ回数上限制約**＝enemy `abilCapPerTurn:19`＋探索候補生成で per-turn `T.ability` 剪定（計数＝手動+再発動は数える／アリアン自動は数えない・**制約であってkill_turn非該当**・シムは火力最適化で生存は目的外）。abi上限/補正率70%解釈は第1走で実測。
