...
> [!NOTE]
> **現状（2026-06）**: Phase 2（エンジン汎用化）完了、Phase 3（高速化）も第一手（_stepStatic/_candidatesアロケーションフリー化：~17%短縮、ゴールデン値検証済）を完了したため、保守性を最優先して早期クローズ・アーカイブ化済み。
> 本書は引き続き**長期ビジョン**（敵行動・味方生存シミュレーション）と**新キャラ導入ワークフロー**の構想を規定するとともに、次の**Phase 4 (実機較正の反復と最適押し順改善の泥沼)** を定義する。`tools/add_character.js` 等の生成器は未実装の将来計画。
...
3. **計画・検証策定（Antigravity）**: 設計エージェントが不整合の原因（パラメータ較正不足や状態遷移バグ）を特定し、`implementation_plan.md` を作成して再現テストケースを定義。
4. **自律修正とテスト（Claude Code）**: 実装エージェントが計画書に基づきコードを修正し、テストを実行。基準値（`91,723,594`）と新規アサートの双方を検証して完了。
