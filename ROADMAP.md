...
> - **Phase 7 = アクセサリー実装** → **Phase 8 へ繰り下げ**。
> - 旧 Phase 6 の残り（敵行動・味方生存／最速撃破モード／VM・ワークフロー）は **未確定Phase**（採番待ち）へ退避。
>
> **採番改訂（ユーザー決定）**: **Phase 7 = 静的スコア s の機械学習化**（大規模Phase・新設）。
> 従来の**アクセサリー実装は Phase 8 へ繰り下げ**（内容不変・本書 §3.5）。
> **Phase 7 クローズ**: PoC×2 で安価サロゲート ML 最適化は **NO-GO**（既存 grid 較正で飽和）と
...
| **7** | **静的スコア s の機械学習化**（探索ヒューリスティックの学習最適化） | **クローズ・archive**（PoC×2で安価サロゲートNO-GO＝既存grid較正で飽和） | `archive/PHASE7_ML_PLAN.md`・本書 §3 |
| **8** | **アクセサリー実装**（新規装備系統） | 計画策定済み・実装未着手（**PHASE8_PLAN.md**） | 本書 §3.5 → PHASE8_PLAN.md |
| **9** | **実機観測 intake の自動化**（録画 → 検証済みデータ） | **← 進行中**（P0 完了／**P1 ゲート待ち**）。**sim05 を凍結して先行** | `PHASE9_PLAN.md`・`TRANSCRIPTION_DESIGN.md` |
| 未確定 | 敵行動・味方生存 ／ 最速撃破 ／ ワークフロー（**intake 部分は Phase 9 が吸収**） | 採番待ち（本書 §4） | 本書 §4・`KILL_TURN_DESIGN.md` |

**現行ゴールデン値**: **[CLAUDE.md](CLAUDE.md)「検証方法」が正**（同じ値を複数の md に置かない＝REPO_STANDARDS §7.2）。
⚠ 本行は**参考**。正は `CLAUDE.md`「検証方法」と `CALIBRATION_ANALYSIS.md`。
...
**実戦に近いシミュレーションへの一歩**（召喚時効果まで配線できれば再現度が大きく上がる）。設計相談の結論＋
**実機仕様の確定**を記録する。

**実機の幻獣構成（確定）**: 編成は **メイン幻獣1体 ＋ サブ幻獣5体 ＋ サポート幻獣1体（フレンドの幻獣）** の計7体。
...
**UI方針（確定）**: 神姫と同様に幻獣を `SUMMON_REGISTRY` へ複数DB登録し、**編成内容に応じてプルダウンで選択**する
...
**状態: クローズ（PoC×2 で NO-GO → archive）。一次情報は `archive/PHASE7_ML_PLAN.md`**。
...
**状態: 計画策定済み・実装未着手（旧 Phase 7 から繰り下げ・詳細は `PHASE8_PLAN.md`）**。
...
## 3.6 Phase 9：実機観測 intake の自動化（**新規・2026-08-07 採番** → PHASE9_PLAN.md）

**解く問題**: **転記コストが較正全体の律速**になっている（`TRANSCRIPTION_DESIGN.md` §0）。
sim05 は残り約10走で、**C40 / C41 / C44 はすべてデータ量に依存**するが、
1 trial あたり「5分の録画をコマ送りで手入力」では走数を増やすほど人が詰まる。

**根本原因＝分担が逆だった**: 人間しかできないこと（実機の観測・判断）と機械がやるべきこと（転記・検算・整形）が混ざり、
**人間が機械的作業を担っていた**。∴ 本 Phase の憲法は
「**観測と判断はユーザー／導出・分析は Claude Code／転記・検算・整形はツール**」。

- **スコープ**: 録画 → 抽出 → 検算 → trial データ生成 ＋ config 版管理。**較正・エンジンは含まない**（Phase 4 のまま）。
- **段取り**: P0 先行整備（✅完了）→ **P1 実測ゲート（★進む/降りるの分岐）** → P2 抽出基盤（AI不要）→ P3 OCR＋検算 → P4 config 版管理 → P5 受入。
- **⚠ sim05 は凍結**（M3〜M6 の実走・C40/C41/C44 は open のまま＝意図的に受け入れたコスト）。**録画は貯めてよい**。
- **方針**: 進行スピードより**網羅性・正確性**を優先（ユーザー決定）。想定外の追加ツールは `PHASE9_PLAN.md` §7 の手順で受け入れる。
- **詳細は `PHASE9_PLAN.md` が正**（本節は索引）。
...
新神姫/英霊/敵の導入は、下記 **md-first intake フロー**を正とする（アリアンロッド・両面宿儺で確立）。
...
| 日付 | 変更点 | 波及確認 |
|---|---|---|
| 2026-08-07 | **Phase 9（実機観測 intake の自動化）を採番**＋§3.6 を新設。§4(iii) の intake 部分を吸収 | ユーザー承認済。**sim05 を凍結して先行**＝C40/C41/C44 は open のまま（意図的コスト）。一次設計 `TRANSCRIPTION_DESIGN.md` を archive から解凍し C49 由来の陳腐化を訂正。走の無効化条件は **REPO_STANDARDS E11** へ |
| 2026-08-05 | 採番経緯の日付を削除。**現行ゴールデン値の記載が2世代前だった**ため CLAUDE.md への参照へ差し替え | 値の正は CLAUDE.md「検証方法」1箇所に集約（REPO_STANDARDS §7.2） |
