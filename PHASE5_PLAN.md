...
*   **二重実装の不整合バグリスク**:
    もしエンジンをJS/WASMで二重実装（探索はWasm、表示や簡易検証はJSなど）する場合、ゴールデン値（`155,756,325`）の挙動完全保存を異なる言語間で恒常的に担保し続けることは、実質的に不可能に近い保守コストを要します。
...
*   **非目標**: 
    探索アルゴリズム・ビーム幅・火力モデル・ゴールデン値（`155,756,325`）には触れない。**挙動・数値は完全保存**。
    - ∴ 本フェーズの全コミットで検証ワンライナー（[CLAUDE.md](file:///c:/Users/Kanta%20Miyanaga/kamipro/CLAUDE.md) §検証方法）が **`155,756,325` 不変**であることをアサートする。
...
*   **S5（開発環境の移行）**:
    ...
    - バンドラを通した最終出力（`dist/`）で、検証ワンライナーが **`155,756,325` 不変**で実行・パスできることを徹底検証する。
...
*   **Worker 再現**: `document` 無しサンドボックスで `init`→`root`→`baseline` が 155,756,325 を返すこと。
