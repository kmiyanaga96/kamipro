...
| **`t1_scene_probe.mjs`** | ★**「ラベルをマスクすると誤読が減るか」を実走フレームで測る**（P3-1b の残り）。入力＝ページの「★ダメージ枠をJSONで保存」が吐く **`kind: 'scene-frame'`**（`dmg` 枠まるごと＋**見えている数値**）。<br>`readScene`（ラベル検出 → マスク → 読み取り）を通し、**マスク有無の両方**を真値と突合して差を出す。<br>⚠ **数値だけ／ラベルだけの切り抜きでは測れない**＝ラベルは数値の**真上**に出るので位置関係が要る。<br>⚠ 真値が空の JSON は**検出の確認までしかできない**（「減ったか」は測れない）。 |
