...
> **新規ドキュメント作成・新規タスク着手の前に本書 §1 の振り分け表を必ず通すこと。**
...
| 場所 | 責務 | 例 |
|---|---|---|
| root `*.md` | **現役の**規定・台帳・計画（クローズしたら archive/ へ git mv。キャラ登録較正はキャラ md §3 へ） | CLAUDE.md・CALIBRATION_ANALYSIS.md |
| `workspace/` | **起動時必読の変動情報**（現状スナップショット＋次タスク・有界） | HANDOFF.md・TODO.md |
...
| 接頭辞 | 意味 | 番号スコープ・台帳 | 状態 |
|---|---|---|---|
| **Cx** | 較正乖離（実機vsシム） | **全リポ一意**・CALIBRATION_ANALYSIS.md §4（採番の正はそこ） | 現役 |
| **Dx** | 確定較正値・設計決定 | **全リポ一意**・CALIBRATION_ANALYSIS.md §2〜3（採番の正はそこ） | 現役 |
...
| **Ex** | **実験・計測の作法（手順規律）** | **本書 §6「実験・計測を回すときの作法」内で連番**。引用は `REPO_STANDARDS E1` 等 | 現役（⚠**Cx/Dx とは別軸**＝Cx=モデルの乖離／Ex=**作業手順の失敗から得た規律**。実験の数値そのものは simNN/ へ、規律だけを Ex に置く） |
...
| 日付 | 変更点 | 波及確認 |
|---|---|---|
| 2026-08-05 | 制定日などの経緯を削除。**§3 の ID レンジ（C1〜C35 / E1〜E8）が陳腐化していた**ため件数を廃し台帳ポインタへ | 件数を持たせない形に変え、同種の陳腐化を構造的に止めた |
...
- **★末尾ブロックは「本文外のメタ領域」と定義する**（ユーザー承認済み）。∴ **一次情報の「原文ママ・本文は書き換えない」原則と抵触しない**。
...
**起動時（読むのは2本のみ）**:
1. **CLAUDE.md（自動ロード）＋ `workspace/HANDOFF.md`（現状スナップショット）** を読む。次タスクは `workspace/TODO.md`。他doc（sim/CALIBRATION/ROADMAP/Phase/essays/キャラmd）は HANDOFF・CLAUDE のポインタ経由で**必要時のみ**読む（新セッション移行のトークン浪費を避ける）。過去の経緯は `archive/SESSION_LOG.md`。
2. 着手前に本書 §1 で分類。分類が曖昧・複数フロー跨ぎ・新形式が必要 → **ユーザーへ選択肢つきで確認**（例:「これは Cx 起票＋simNN 新設のどちらで進めますか」）。

**実験・計測を回すときの作法（同じ時間の溶かし方を繰り返さないため）**:
...
| E9 | **エジソン編成で確立した機構・定数は、新編成では必ず再測する**（一般則として書けるのは**2編成以上で再現**したときだけ）。原則と該当事例＝**[ENGINE_INVARIANTS.md](ENGINE_INVARIANTS.md) §3**（CLAUDE.md 開発ルール §5 は要約） | `BEAM_W=64`(C16)／Phase7 §7.2／`CALIB_GRID`／C27リファイン の**4例**が同じ形で外れた |
...
| **E11** | **★走を無効化する config 変化には数値の閾値を置く。「変わったから取り直す」を禁じる。**<br>判定は**実機の表示ATK 5人分**を基準に行う（GEAR/幻獣/サブ枠は別扱い＝下表）。<br><br>**続行してよい**: ①**プレイヤーランクの変化**（何ランクでも。10T 総ダメージで +0.006% ＝実機ばらつき CV 1.04% の **1/200**）②**表示ATK が5人とも ≦1%**（+0.28%＝ノイズの27%）。**trial に記録だけして走は続ける**。<br>**走を分ける**（config 版を切る）: ③**表示ATK が5人とも >2%** ④**GEAR の変化**（装備パネルが動いた） ⑤**幻獣枠・サブ枠の変化**。<br>⚠ **ランクは英霊にしか掛からない**（`calcDisplayAtk` の `def.type==='hero'` 分岐＝神姫4人は非適用）。<br>⚠ **マスターボーナスを割り振った場合は式で外挿せず実機の表示値を読む**（シムは非適用）。 | **M3 の実機測定が、録画中のランクアップを理由に丸ごと中断・再取得された**（2026-08-07）。実測すると影響は**ノイズ床の 1/1000**＝**取り直しは不要だった**。原因は「ランクが上がったこと」ではなく**何が走を無効化するのかが決まっていなかったこと**で、基準が無いので安全側に倒すしかなかった。⚠ **一般化**: 判定基準の無い変数は、**必ず過剰反応か見落としのどちらかを生む**（幻獣枠は見落とし側の同型事例）。感度は `PHASE9_PLAN.md` §4／実測は `tools/` の一時スクリプトで再現可能 |
...
6. **★md リレーション点検（常駐サブタスク）**: `workspace/TODO.md` 冒頭のカウンタを **+1** する。
...
## 7. 参照の統一文言（[DOC_RELATION_PLAN.md](DOC_RELATION_PLAN.md) §4 が設計の正）
