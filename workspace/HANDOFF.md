...
>
> 最終更新: 2026-08-14（**P1 通過 ＋ P2-1/P2-3 完了**・次は P2-2/P2-4/P2-5）
...
## ⏳ 次の一手 ── **P2-2 / P2-5（Claude 作業）**

**P2-1・P2-3 は完了**。残りは **P2-2 フレーム抽出** → **P2-4 重複除去** → **P2-5 HP%＋CT 抽出**。
**出口条件**＝`PHASE9_PLAN.md` §4 P2（既知の1走で人が見るフレーム数が1桁減り、HP% が自動で取れること）。

⚠ **ユーザー作業として残っているのは `gauge` クロップ PNG 1枚だけ**
（5キャラぶんが1フレームに同時に映るか／選択キャラのみか＝**C24 の観測経路の形が決まる**）。
...
| **P2** | ⏳ **現在ここ** | **P2-1 ✅完了**（実フレーム検証済）／**P2-3 ROI 採寸 ✅完了**（5枠固定）／P2-2・P2-4・P2-5 未着手 |
...
| ライン | 状態 | 次アクション |
|---|---|---|
| **★Phase 9** | ⏳ **P2 抽出基盤** | **P2-2 フレーム抽出**から（Claude・AI 不要）。ユーザー側の残りは `gauge` クロップ1枚 |
