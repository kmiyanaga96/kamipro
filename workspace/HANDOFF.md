...
## ⏳ 次の一手 ── **P2 抽出基盤（Claude 作業・AI 不要）**

**最初に作るのは `canvas 正規化`**（P1 新発見②＝**固定クロップが不可**と判明したため。全 ROI 由来の検算の前提条件）。
→ フレーム抽出 → ROI クロップ → 重複除去 → HP% 検算。**出口条件**＝`PHASE9_PLAN.md` §4 P2。
...
| **P2** | ⏳ **現在ここ** | **canvas 正規化**（最初）→ フレーム抽出 → ROI クロップ → 重複除去 → HP% 検算 |
