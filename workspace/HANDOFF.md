...
## ⏳ 次の一手 ── **ROI 採寸（ユーザー作業・10分）**

⚠ **作業ブランチにしか無い**: `git checkout claude/session-handoff-xf3up8` → `npm install` → `npm run dev`
→ **`http://localhost:5173/transcribe/`**（`/` はシム本体のまま）。

1. 録画を開く →「このフレームを解析」
2. **ダメージが出ているフレーム**で、下の5つを順にドラッグ →「この矩形を登録」
   | name | 何を囲むか |
   |---|---|
   | `hp` | 敵HPバー＋`100%` 表示 |
   | `dmg` | ダメージ／`TOTAL` ポップアップが出る範囲全体 |
   | `abil` | 右のアビリティパネル（4枠＋CD残ターン） |
   | `turn` | `NEXT n TURN` |
   | `gauge` | キャラ Status（HP・バーストゲージ） |
3. **「全ROIをJSONでコピー」して貼る**
4. あわせて **「連続フレームの間隔を実測」**（**P1 発見⑧＝ポップアップの寿命**の測定）
5. 可能なら `hp` と `dmg` を **「この矩形をPNGで保存」**して添付（§10.3＝Claude が中身を見られる唯一の経路）
