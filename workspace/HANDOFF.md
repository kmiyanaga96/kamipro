...
## ⏳ 次の一手 ── **M2 の転記 → M5 → M3 → M4**（★具体手順は [workspace/TODO.md](./TODO.md) 冒頭）

**★M2 は「録画済み・転記待ち」**（ユーザー報告 2026-08-05）。動画は取れているが
`simulation/sim05/data/trialNN.md` への転記が未了＝**Claude Code はまだ分析できない**。
**次セッションの最初の一手＝録画から trialNN.md への転記**。それまで A トラックは動かない。
⚠ **`record_skeleton.md` は 2026-08-05 にキャスパリーグ用へ更新済**（宿儺専用は §6 付録へ降格／
**押下ごとの契晶列**と**バーストボタン ON/OFF 欄**を新設／メニュー別の必須欄 §4）。
⚠ 転記が重いなら `archive/TRANSCRIPTION_DESIGN.md`（半自動化・**凍結中**）の再起票が選択肢
＝**クリティカルパスに乗った**。凍結解除はユーザーの判断。
...
| トラック | 内容 | ゲート |
|---|---|---|
| **A. ダメージ較正** | **C40 / C41 / C44**（＋C42/C43）／C3/C5 は縮小／C45/C46 は新規 | ⏳ **M2 の転記 → M5→M3→M4**（宿儺での fit は変数過多＝G4.9 へ後ろ倒し） |
...
| ライン | 状態 | 次アクション |
|---|---|---|
| **★M2 の転記 → M5→M3→M4** | ⏳ **M2 は録画済み・転記待ち＝これが唯一のブロッカー** | 録画から `trialNN.md` へ転記（skeleton 更新済）。＋`configC_slot.json`。手順は TODO 冒頭 |
