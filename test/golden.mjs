...
// 現在値: C19(tenya_re=赤アビ使用扱い)/C20(ifishant→エレイン+1リキャスト)実機較正で再fit
//   （2026-07-09・override {judg:160,pactcore:1,effond:100}）。
...
setStaticOverride({});
const raw = run10T();
const rawOk = raw.dmg === 195518168 && raw.fb === 10;

// 自動較正 override（本編成の calibrateStaticScores 選択結果 = {judg:160,pactcore:1,effond:100}・C19/C20）を適用した production 値
setStaticOverride({ judg: 160, pactcore: 1, effond: 100 });
const cal = run10T();
setStaticOverride({});
const calOk = cal.dmg === 212010783 && cal.fb === 10;

const ok = rawOk && calOk;
console.log(`[golden] raw=${raw.dmg} FB=${raw.fb}/10 | calibrated=${cal.dmg} FB=${cal.fb}/10 => ${ok ? 'OK' : 'MISMATCH (expect raw 195518168 / calibrated 212010783 / 10)'}`);
process.exit(ok ? 0 : 1);
